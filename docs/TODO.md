...
- [ ] Server functions + React Query hooks for bill domain (pay schedules, bills, bill instances)
  ...
  - [ ] **Pay schedules** — only `listPaySchedules` + `usePaySchedules` exist ([pay-schedules-service.ts](src/features/pay-schedules/pay-schedules-service.ts)). Still need:
    - [ ] `pay-schedules-model.ts` with `PaySchedule` type re-export and `createPayScheduleSchema` / `updatePayScheduleSchema` (drizzle-zod, matching the bills pattern)
    - [ ] `createPaySchedule`, `updatePaySchedule`, `archivePaySchedule` server functions + matching mutation hooks (`useCreatePaySchedule`, etc.) with cache invalidation
  ...
  - [ ] **Bill instances** — nothing exists yet; instance reads are only bundled into `getBillDetail`. Still need (all under `features/bills/`):
    - [ ] `bills-helpers.ts` with pure date math: `clampDayToMonth(day, year, month)` wrapping `Math.min(daysInMonth, day)`, and `computeNearestUnpaidDueDate(bill, existingInstances, today)` implementing the unbounded look-ahead from CLAUDE.md
  ...
  - [ ] **State derivation** — `deriveBillState(bill, instances, today)` in `bills-helpers.ts` returning the `PAID | OVERDUE | MISSED_SCHEDULE | UPCOMING` union from CLAUDE.md (frontend-only, O(1) Map lookup keyed on `billId`)
